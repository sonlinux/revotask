...
def create_user(payload: UserSchema, session: Session = Depends(get_session)):
    # validate_username = has_letters_only(user.username)
    # validate_date = is_past_date(user.birthdate)
    validate_username = payload.username
    validate_date = payload.birthdate
    if validate_username and validate_date:
...
def get_user(username: str,  session: Session = Depends(get_session)):
    ...
    if not user_obj:
        raise HTTPException(status_code=status.HTTP_204_NO_CONTENT, detail=f"204 No Content")
    
    days_to_birthday = days_left(user_obj.birthdate)

    if days_to_birthday > 0:
        return f"Hello, {user_obj.username}! Your birthday is in {days_to_birthday} day(s)"
    else:
...
def update_user(payload: UserSchema,  session: Session = Depends(get_session)):
    ...
    if not user_obj:
        raise HTTPException(status_code=404, detail=f"user item with name {payload.username} not found")

    update_data = payload.dict(exclude_unset=True)
    user_query.filter(User.username == payload.username).update(update_data, 
